...
#define TM_97C      FROM_DEGREE(97.0)
#define TM_100C     FROM_DEGREE(100.0)
  ...
  switch (state)
  {
    ...
    case PCS_STARTING: // Начинаем цикл
      ...
      else
      {
        ...
        //если мы греем меньше 100градусов или уже нагрето больше 100градусов, то сразу переключаемся на предварительный нагрев
        else if (targetValue < TM_100C || realValue > TM_100C)
        {
    ...
    case PCS_DRY:
      ...
      {
        if (_checkWarmUp(realValue, intValue, TM_DRY)) //если еще не нагрели, то пытаемся греть...
        {
